...
- **Rate limiting**: `bucket4j` + Redis (`ratelimit/RateLimitFilter`, distributed via a `LettuceBasedProxyManager`), applied per-surface: per-tenant on `/mcp/**`, per-IP on `/oauth2/token`+`/oauth2/authorize` and on `/login`+`/signup` POSTs. Redis is load-bearing for this — see `docker-compose.yml`.
- **Circuit breaking**: `resilience4j` wraps `ImapMailService.withStore`, one `CircuitBreaker` per `(tenantUserId, imapAccountId)` (same granularity as the connection pool's own key). Only `ImapMailService.ImapOperationException` counts toward the failure rate (`resilience4j.circuitbreaker.configs.default.record-exceptions` in `application.yml`) — a bad folder/UID (`NoSuchElementException`) is a client error, not upstream flakiness, and must never trip it.
- **Audit log**: `io.imapmcp.audit.AuditLogService` writes one `audit_log` row per tool call (success/error/denied) and per account lockout trip. Write-only — no read API yet. A write failure is logged and swallowed, never propagated (an audit-trail gap must not break the underlying tool call).
- **Metrics**: `spring-boot-starter-actuator` + Micrometer/Prometheus, served on a **separate `management.server.port`** (default `8081`) that must never be published externally in a deploy — see the gotcha below on why that alone isn't real isolation from the web `SecurityFilterChain`. Custom metrics: `mcp.tool.calls` (timer, tags `tool`/`result`), `ratelimit.requests` (counter, tags `surface`/`outcome`), `imap.pool.active`/`imap.pool.idle` (gauges), `imap.account.lockouts` (counter). Circuit breaker state/transitions are published automatically by `resilience4j-spring-boot3`'s Micrometer autoconfiguration once both actuator and Micrometer are present.
- **Structured logging**: `logback-spring.xml` — plain console pattern under the `local` profile, JSON (`LogstashEncoder`) otherwise. `RequestCorrelationFilter` (a genuine `@Component`, unlike the security filters below — see its javadoc) puts a request ID in MDC ahead of all three `SecurityFilterChain`s, including auth failures; `TenantContextFilter` adds `tenantId` to MDC alongside its existing `TenantContext::set` call.

### What's still stubbed vs. not yet built
...
- No read API for `audit_log` — query it directly (`psql`/DB tooling) until one exists.
...
- **A separate `management.server.port` does not exempt actuator from the app's own `SecurityFilterChain`s.** Expected (and cheaper) isolation would be a distinct child security context that ignores the main app's `authorizeHttpRequests` rules entirely; empirically here it doesn't — a request to `/actuator/prometheus` on the management port still hit `SecurityConfig`'s `anyRequest().authenticated()` and got redirected to `/login`. Fix was the mundane one: add `/actuator/health` and `/actuator/prometheus` to that chain's `permitAll`, same as any other unauthenticated path, and treat the separate port as network-level isolation only (never publish it) rather than an application-level auth boundary.
